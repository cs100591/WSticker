...
interface LocalStore {
  ...
  // Note actions
  addNote: (noteData: Omit<Note, 'id' | 'createdAt' | 'updatedAt'> & { id?: string; createdAt?: string; updatedAt?: string }) => Note;
  ...
  persist(
    (set, get) => ({
      ...
      updateNote: (id, updates) => {
        set((state) => {
          const updatedNotes = state.notes.map((n) =>
            n.id === id ? { ...n, ...updates, updatedAt: updates.updatedAt || now() } : n
          );
      ...
      deleteNote: (id) => {
        set((state) => {
          const updatedNotes = state.notes.map((n) =>
            n.id === id ? { ...n, isDeleted: true, updatedAt: now() } : n
          );
          return { notes: updatedNotes };
        });
        import('@/services/sync/SyncManager').then(({ syncManager }) => syncManager.sync().catch(console.error));
      ...
      getNotes: () => get().notes.filter((n) => !n.isDeleted),
      setNotes: (notes) => {
        set({ notes });
      },
      ...
      // Utility
      clearAll: () => {
        set({ todos: [], expenses: [], calendarEvents: [], notes: [] });
